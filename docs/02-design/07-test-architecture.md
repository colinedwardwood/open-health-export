...
| **R-31 for the companion** | TLS 1.2 PSK (`TLS_PSK_WITH_AES_128_GCM_SHA256` on Network.framework; TN3213 has no TLS 1.3 PSK) with a 256-bit PSK established by QR pairing, plus a six-digit confirmation code derived from the TLS key exporter shown on both screens. Mutual authentication, no CA, no pin-on-first-use ambiguity. The PSK-only ciphersuite does not provide forward secrecy. Multipeer Connectivity rejected explicitly *because* peer identity is a display name — the RK-11 argument I was going to make | I had "no durable peer identity" as a **Major**. Withdrawn. The Darwin PSK API is weaker than TLS 1.3 PSK-(EC)DHE on forward secrecy; that is a platform limit, not a design preference |
