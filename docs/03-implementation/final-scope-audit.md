...
- The iPhone exporter uses four root tabs in fixed order: Status, Data, Destinations, History. Destinations stays second from the right (R-41). Settings opens from the Status toolbar and shows the data-flow explainer plus a Destinations link.
- The status widget maps WidgetKit `fullColor` / `accented` / `vibrant` chrome onto glyph-and-label identity and uses monochrome symbols when hue is stripped (UX-41). Greyscale screenshot measurement remains a later gate.
