...
- The status widget maps WidgetKit `fullColor` / `accented` / `vibrant` chrome onto glyph-and-label identity and uses monochrome symbols when hue is stripped (UX-41). When the device is locked it redacts to a lock glyph (SEC-28). Greyscale screenshot measurement remains a later gate.
