...
`_ohx-recv._tcp` name with TLS 1.2 PSK (Network.framework; Apple TN3213 has no TLS 1.3 PSK); discovery is not authorization, only an exact match on
...
can push one page over TLS-PSK to the exact Bonjour name captured at pairing. Pairing
