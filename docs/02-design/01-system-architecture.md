...
| **M11** | **Compliance, governance, release engineering** | 3 | 83 | **R-109** medical-device declaration visible before submission; "not a medical device" in first-run, About, README and landing page. **R-111** DSA trader status declared against the legal entity (D-13). **R-105a/R-105b** are not pursued under D-10's owner-directed model. **R-106** `CONTINUITY.md` reviewed by someone outside the project. **R-107** machine-readable maintenance status and OS matrix. **R-108** a person unaffiliated with implementation builds from a tag using only the README. **R-110** feature-flag audit shows no sponsor-gated capability. **R-112** written opinion on file. **R-113** denylist check over published copy, zero matches |
