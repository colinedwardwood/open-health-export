...
enum SHA256 {
    ...
    struct Hasher {
        ...
        /// Drop finished blocks in one memmove rather than shifting the buffer per block.
        mutating func drainFullBlocks() {
            var offset = 0
            while buffer.count - offset >= 64 {
                compress(from: offset)
                offset += 64
            }
            if offset > 0 {
        ...
        private mutating func compress(from offset: Int) {
            let k = Self.roundConstants
            var w = [UInt32](repeating: 0, count: 64)
            for i in 0..<16 {
                w[i] = UInt32(buffer[offset + i * 4]) << 24
                    | UInt32(buffer[offset + i * 4 + 1]) << 16
                    | UInt32(buffer[offset + i * 4 + 2]) << 8
                    | UInt32(buffer[offset + i * 4 + 3])
            }
            for i in 16..<64 {
